...
def check_data_distribution(data, feature_columns, categorical_features):
    ...
    fig.tight_layout()
    fig.savefig(f"{plot_path}/Histogram_with_KDE.png")
    ...
    fig.tight_layout()
    fig.savefig(f"{plot_path}/Box_and_whisker.png")
...
def plot_feature_importances(importances, title):
    ...
    fig.tight_layout()
    plot_path = './plots'
    fig.savefig(f"{plot_path}/{title}.png", dpi=300,
...
def visualize_predictions(data):
    ...
    sns.barplot(x='Period', y='Total Spend', data=comparison_data, ax=ax)
    ax.set_title(
    ...
    fig.tight_layout()
    plt.savefig(f"{plot_path}/total_income_comparison.png")
    plt.show()


def main():
    training_data = pd.read_csv(f"{csv_dir}/clv_featured_clustered.csv")
    complete_dataset = pd.read_csv(f"{csv_dir}/data_featured_clustered.csv")

    feature_columns = [
        'RFM_Level', 'ClusterGroup', 'R_Score', 'F_Score',
        'M_Score', 'Quantity', 'Price', 'Month', 'DayOfWeek', 'TimeOfDay'
    ]
    ...
    #     training_data, feature_columns, categorical_features)
    ...
    check_data_distribution(
        complete_dataset, feature_columns, categorical_features)

    results = make_predictions(complete_dataset, feature_columns)
